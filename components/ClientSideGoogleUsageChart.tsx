...
export default function ClientSideGoogleUsageChart({ usageStats }: { usageStats: TokenUsageLog[] }) {

  const chartData: ChartDataItem[] = useMemo(() => {
    const totalPromptTokens = usageStats.reduce((sum, log) => sum + (log.token_count.google?.promptTokens || 0), 0);
    const totalCompletionTokens = usageStats.reduce((sum, log) => sum + (log.token_count.google?.completionTokens || 0), 0);
  ...
  const totalTokens = useMemo(() => 
    usageStats.reduce((sum, log) => sum + (log.token_count.google?.totalTokens || 0), 0),
    [usageStats]
  );
